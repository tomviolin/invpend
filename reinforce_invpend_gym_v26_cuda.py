...
class Policy_Network(nn.Module):
    ...
    def __init__(self, obs_space_dims: int, action_space_dims: int):
        ...
        hidden_space1 = int(read_hyper('hidden_space1',16 )) # Nothing special with 16, feel free to change
        hidden_space2 = int(read_hyper('hidden_space2',32 )) # Nothing special with 32, feel free to change
...
class REINFORCE:
    ...
    def __init__(self, obs_space_dims: int, action_space_dims: int):
        ...
        # Hyperparameters
        self.learning_rate = float(read_hyper("learning_rate", 1e-4))  # Learning rate for policy optimization
        print(f"** learning rate = {self.learning_rate:0.6f}")
        ...
        self.net = Policy_Network(obs_space_dims, action_space_dims).cuda()
    ...
    def sample_action(self, state: np.ndarray) -> float:
        ...
        action_means, action_stddevs = self.net(state.cuda())
    ...
    def update(self):
        ...
        deltas = torch.tensor(gs).cuda()

        log_probs = torch.stack(self.probs).cuda()

        # Calculate the mean of log probabilities for all actions in the episode
        log_prob_mean = log_probs.mean().cuda()
        ...
        loss = -torch.sum(log_prob_mean * deltas).cuda()
...
gym.envs.register(
    ...
    max_episode_steps=MAX_EPISODE_STEPS,  # Set the maximum number of steps per episode
    reward_threshold=1900.0,  # Set a reward threshold for success
)
...
wrapped_env = gym.wrappers.RecordEpisodeStatistics(env, 1000)  # Records episode-reward

total_num_episodes = int(TOTAL_NUM_EPISODES)  # Total number of episodes
...
for seed in [int(read_hyper("seed", 1234)),]:  # Fibonacci seeds
    ...
    if os.path.exists(dict_file):
        ...
        # torch.save(agent.net.state_dict(), f"reinforce_invpend_gym_v26_seed_{seed}.pth")
    ...
    for episode in range(total_num_episodes):
        ...
        try:
            while not done:
                ...
                agent.rewards.append(reward) # Store the reward scaled by the number of steps taken so far.
                #print(f"Step {numsteps}: Action: {action}, Reward: {reward}, Obs: {obs[3]}")
    ...
    torch.save(agent.net.state_dict(), os.path.join(datadir,f"reinforce_invpend_gym_v26_seed_{seed}.pth"))
    print(f"Seed {seed} completed. Average reward over episodes: {np.mean(reward_over_episodes)}") 
# Save the agent's policy network for later use
torch.save(agent.net.state_dict(), os.path.join(datadir,"reinforce_invpend_gym_v26.pth"))
...
totaldatapoints=df1.shape[0]
df1_firstthird = df1[:totaldatapoints//3]
df1_lastthird = df1[-(totaldatapoints//3):]

plt.plot(df1['episodes'],df1['reward'],'-',label='all')
plt.plot(df1_firstthird['episodes'],df1_firstthird['reward'],'-',label='firstthird')
plt.plot(df1_lastthird['episodes'],df1_lastthird['reward'],'-',label='lastthird')
plt.title(read_hyper("title", f"Learning Rate = {float(read_hyper("learning_rate", 1e-4)):0.6f}\nSeed:{int(read_hyper("seed", 1234))}"))
...
sns.lineplot(x="episodes", y="reward",data=firstthird)
sns.lineplot(x="episodes", y="reward",data=lastthird)
"""

meanfirstthird = np.mean(df1_firstthird['reward'])
meanlastthird = np.mean(df1_lastthird['reward'])

plt.plot((df1_firstthird['episodes'][0],
          df1_firstthird['episodes'][totaldatapoints//3-1]),
        (meanfirstthird,meanfirstthird),'-',label=f'median(first third)={meanfirstthird:4.2f}')

plt.plot((df1_lastthird['episodes'][totaldatapoints-totaldatapoints//3],
          df1_lastthird['episodes'][totaldatapoints-1]),
        (meanlastthird,meanlastthird),'-',label=f'mean(last third)={meanlastthird:4.2f}')




plt.legend()
figfile = "reinforce_invpend_gym_v26_fig4.png";
figfile=os.path.join(datadir,figfile)
figfile = os.path.realpath(figfile)
plt.savefig(figfile)
